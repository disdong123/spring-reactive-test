...
class SlaveWebfluxServerApplication (
    ...
    suspend fun jasyncMono(idx: String): Mono<QueryResult> {
        logger.info("jasyncMono()")
        return Mono.fromFuture(jConnection.sendPreparedStatement("DO SLEEP(1)"))
    }

    @GetMapping("/jasync-coroutine")
    suspend fun jasyncCoroutine(idx: String): QueryResult? {
        logger.info("jasyncCoroutine()")
        return jConnection.sendPreparedStatement("DO SLEEP(1)").asDeferred().await()
    }
}
